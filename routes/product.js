...
router.post("/add", auth.authenticate, role.checkRole, (req, res) => {
  ...
  let query =
    'insert into product (name, categoryID, description, price, stockQuantity, status) values(?,?,?,?,?,"true")';

  connection.query(
    query,
    [product.name, product.categoryID, product.description, product.price, product.stockQuantity],
    (err, results) => {
      if (!err) {
...
router.get("/get", auth.authenticate, (req, res, next) => {
  let query =
    "select p.id, p.name, p.description, p.price, p.stockQuantity, p.status, c.id as categoryID, c.name as categoryName from product as p INNER JOIN category as c where p.categoryID=c.id ";
...
router.patch("/update", auth.authenticate, role.checkRole, (req, res, next) => {
  let product = req.body;
  let query =
    "update product set name=?, categoryID=?, description=?, price=?, stockQuantity=? where id=?";
  connection.query(
    query,
    [
      product.name,
...
router.patch("/updateStatus", auth.authenticate, role.checkRole, (req, res, next) => {
